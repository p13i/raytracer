...
bool geo::intersection(LineSegment j, LineSegment k, Point& intersectionPoint) {
  ...
  float denominator =
      linalg::Matrix2x2(x1 - x2, x3 - x4, y1 - y2, y3 - y4).determinant();

  if (denominator == 0.f) {
    return false;
  }

  float t = linalg::Matrix2x2(x1 - x3, x3 - x4, y1 - y3, y3 - y4).determinant();

  t /= denominator;

  float u = linalg::Matrix2x2(x2 - x1, x1 - x3, y2 - y1, y1 - y3).determinant();
...
float geo::dist(Point a, Point b) { return Vector(a, b).magnitude(); }
...
float geo::dot(const Point& a, const Point& b) { return a.x * b.x + a.y * b.y; }
...
Ray geo::reflect(Vector forwardTrace, LineSegment intersectedEdge) {
  ...
  auto n = normal(intersectedEdge);
  auto dot = geo::dot(forwardTrace.unit().dest, n.unit().dest);
  auto r = d - 2.f * n * dot;
  r = r.unit();

  return Ray(forwardTrace.dest, r);
}
...
bool geo::intersection_of_rays(Ray ray1, Ray ray2, Point& intersection_point) {
...
bool geo::point_between_rays(const Ray& a, const Ray& b, const Point& p) {

...
Point rt::Origin(const UnboundBeam u_beam) {
  const Ray bound_a_reverse = geo::rotate(u_beam.bound_a_, RT_PI);
  const Ray bound_b_reverse = geo::rotate(u_beam.bound_b_, RT_PI);
  Point intersection;
  geo::intersection_of_rays(bound_a_reverse, bound_b_reverse, intersection);
  return intersection;
}
...
bool rt::Contains(UnboundBeam u_beam, LineSegment line_segment) {
  ...
  Point _;
  bool intersects_left_bound = geo::intersection(line_segment, line_left, _);
  if (intersects_left_bound) {
    return true;
  }

  LineSegment line_right{this_origin, u_beam.bound_a_(10000)};
  bool intersects_right_bound = geo::intersection(line_segment, line_right, _);
...
std::ostream &operator<<(std::ostream &os, const UnboundBeam &u_beam) {
